...
class PhoneDetector:
    ...
    # Risk thresholds
    STATIONARY_THRESHOLD = 3.0  # seconds - reduced for faster alerts
    HIGH_RISK_SCORE = 0.8
    ALERT_COOLDOWN = 5.0  # seconds - prevent duplicate alerts for same zone
    ...
    def __init__(self, hall_name: str = "Cinema Hall 1"):
        ...
        # Store active alerts to prevent duplicates: {zone: last_alert_timestamp}
            ...
            for box in boxes:
                ...
                if class_id == self.PHONE_CLASS_ID:
                    ...
                    if duration >= self.STATIONARY_THRESHOLD:
                        ...
                        last_alert_time = self.active_alerts.get(zone, 0)
    ...
    async def _trigger_alert(self, zone: str, risk_score: float, bbox: Tuple[int, int, int, int]):
        ...
        alert_saved = False
        try:
            result = await save_alert(self.hall_name, risk_score, zone)
            alert_saved = bool(result)  # Empty dict means failure
            if not alert_saved:
                print(f"⚠ Alert NOT saved to database for zone {zone}")
        except Exception as e:
            print(f"⚠ Exception while saving alert to database: {e}")
